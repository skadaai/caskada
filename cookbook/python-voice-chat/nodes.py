...
class CaptureAudioNode(Node):
    ...
    async def exec(self, _): # prep_res is not used as per design
    ...
    async def post(self, shared, prep_res, exec_res):
        ...
        if audio_numpy_array is None:
            ...
            return self.trigger("end_conversation")
...
class SpeechToTextNode(Node):
    ...
    async def prep(self, shared):
        user_audio_data = getattr(shared, "user_audio_data")
        user_audio_sample_rate = getattr(shared, "user_audio_sample_rate")
...
class QueryLLMNode(Node):
    ...
    async def prep(self, shared):
        chat_history = getattr(shared, "chat_history", [])
...
class TextToSpeechNode(Node):
    ...
    async def post(self, shared, prep_res, exec_res):
        ...
        if getattr(shared, "continue_conversation", True):
            return self.trigger("next_turn")
