...
def build_mermaid(start):
    ...
    def walk(node, parent=None, action=None):
        ...
        if isinstance(node, Flow):
            node.start and parent and link(parent, get_id(node.start), action)
            lines.append(
                f"\n    subgraph sub_flow_{get_id(node)}[{type(node).__name__}]"
            )
            node.start and walk(node.start)
            for act, nxt_list in node.successors.items():
                for nxt_item in nxt_list:
                    node.start and walk(nxt_item, get_id(node.start), act) or (
                        parent and link(parent, get_id(nxt_item), action)
                    ) or walk(nxt_item, None, act)
            lines.append("    end\n")
        else:
            lines.append(f"    {(nid := get_id(node))}['{type(node).__name__}']")
            parent and link(parent, nid, action)
            [walk(nxt_item, nid, act) for act, nxt_list in node.successors.items() for nxt_item in nxt_list]
    ...
    def walk(node, parent=None, group=None, parent_group=None, action=None):
        ...
        if isinstance(node, Flow):
            ...
            if node.start:
                ...
                walk(node.start, parent, flow_group, parent_group, action)

                # Process successors of the flow's start node
                for next_action, nxt_list in node.successors.items():
                    for nxt_item in nxt_list:
                        walk(
                            nxt_item,
                            get_id(node.start),
            ...
            for next_action, nxt_list in node.successors.items():
                for nxt_item in nxt_list:
                    if isinstance(nxt_item, Flow):
                        # This node connects to a flow - track the group relationship
                        flow_group_id = get_id(nxt_item)
                        walk(nxt_item, node_id, None, group, next_action)
                    else:
                        # Regular node-to-node connection
                        walk(nxt_item, node_id, group, parent_group, next_action)
