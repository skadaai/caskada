...
```"""
    ...
    async def post(self, shared, prep_res, exec_res):
        ...
        print(f"\n===== GENERATED SQL (Attempt {getattr(shared, 'debug_attempts', 0) + 1}) =====\n")
        print(exec_res)
        print("\n====================================\n")
...
class ExecuteSQL(Node):
    ...
    async def post(self, shared, prep_res, exec_res):
        ...
        if success:
            ...
            shared["debug_attempts"] = getattr(shared, "debug_attempts", 0) + 1
            max_attempts = getattr(shared, "max_debug_attempts", 3) # Get max attempts from shared
            ...
            if shared["debug_attempts"] >= max_attempts:
                ...
                self.trigger("error_retry") # Signal to go to DebugSQL

class DebugSQL(Node):
    async def prep(self, shared):
        return (
            getattr(shared, "natural_query"),
            getattr(shared, "schema"),
            getattr(shared, "generated_sql"),
            getattr(shared, "execution_error")
        )
...
"""
        llm_response = call_llm(prompt)
        assert "```yaml" in llm_response, "Response must contain yaml block"
    ...
    async def post(self, shared, prep_res, exec_res):
        ...
        delattr(shared, "execution_error") # Clear the previous error for the next ExecuteSQL attempt

        print(f"\n===== REVISED SQL (Attempt {getattr(shared, 'debug_attempts', 0) + 1}) =====\n")
        print(exec_res)
        print("\n====================================\n")