...
export class ApprovalNode extends Node {
    async prep(shared: SearchAgentSharedContext) {
        return shared.searchQuery;
    }

    async exec(searchQuery: string) {
        console.log(`Do you allow agent to perform a web search using query: "${searchQuery}" ?`)
        const input = await promptUser()
        return input;
    }

    async post(_shared: SearchAgentSharedContext, prepRes?: string, execRes?: string) {
        if (!prepRes) {
        ...
        if (execRes.toLowerCase() === "yes") {
            return 'search'
        } else if (execRes.toLowerCase() === "no") {
            return 'done'
        } else {
            return 'decide'
        }
