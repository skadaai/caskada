...
# --- Helper functions for running async flows ---
def run_async_initial_flow(session_state):
    async def _run_flow():
        initial_flow = create_initial_processing_flow()
        await initial_flow.run(session_state)
    asyncio.run(_run_flow())

def run_async_finalization_flow(session_state):
    async def _run_flow():
        finalization_flow = create_finalization_flow()
        await finalization_flow.run(session_state)
    asyncio.run(_run_flow())
        ...
        if not task_input_value.strip():
            ...
            try:
                with st.spinner("Processing initial task..."):
                    run_async_initial_flow(st.session_state)
        ...
        if not approve_button_disabled: # Process click only if button was not meant to be disabled
            ...
            try:
                with st.spinner("Finalizing result..."):
                    run_async_finalization_flow(st.session_state)
