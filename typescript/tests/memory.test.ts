...
describe('Memory Class', () => {
  describe('Initialization', () => {
    ...
    it('should initialize with global and local stores', () => {
      ...
      const memory = Memory.create(global, local)
      ...
      assert.deepStrictEqual(
        memory.local,
        { l1: 'local1', common: 'local_common' },
        'Local store should contain initial local data',
      )
    })
  })
  ...
  describe('Proxy Behavior (Reading)', () => {
    ...
    it('should correctly access the local property', () => {
      assert.deepStrictEqual(memory.local, { l1: 'local1', common: 'local_common' })
    })
  })
  ...
  describe('Proxy Behavior (Writing)', () => {
    ...
    beforeEach(() => {
      ...
      memory = Memory.create(global, local)
    })
    ...
    it('should remove property from local store if it exists when writing globally', () => {
      ...
      assert.strictEqual(
        memory.local.common,
        undefined,
        'Accessing via memory.local should also show removal',
      )
    })

    it('should throw error when attempting to set reserved properties', () => {
      assert.throws(() => (memory.global = {}), /Reserved property 'global' cannot be set/)
      assert.throws(() => (memory.local = {}), /Reserved property 'local' cannot be set/)
      // Cannot test setting __global and __local directly as they are private
    })
  })
  ...
  describe('Cloning (`clone()`)', () => {
    ...
    it('should create a deep clone of the local store', () => {
      ...
      assert.notStrictEqual(
        clonedMemory.local,
        memory.local,
        'Local store reference should NOT be shared',
      )
      assert.deepStrictEqual(
        clonedMemory.local,
      ...
      assert.strictEqual(
        memory.l2,
        undefined,
        'Original should not see local changes from clone (reads undefined)',
      )
      assert.strictEqual(
        memory.local.l2,
        undefined,
        'Original local store internal value should be unchanged',
      )
      ...
      assert.deepStrictEqual(
        clonedMemory.nestedL,
        { val: 2 },
        'Nested local object in clone should be unaffected',
      )
    })

    it('should correctly merge forkingData into the new local store', () => {
      const forkingData = { f1: 'forked1', common: 'forked_common', nestedF: { val: 3 } }
      ...
      assert.equal(
        clonedMemory.common,
        'forked_common',
        'Forked data should shadow original local and global',
      )
      ...
      assert.deepStrictEqual(
        clonedMemory.nestedF,
        { val: 3 },
        'Nested object in forked data should have been deep cloned',
      )
    })
